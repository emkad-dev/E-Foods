...
Deno.test('the real domain modules together register all 64 handlers, none pre-auth but the two allowed', () => {
  expectEqual(ALL_RPC_ACTIONS.length, 64, 'ALL_RPC_ACTIONS length');

  const dispatcher = buildDispatcher(REAL_DOMAINS);
  expectEqual(dispatcher.actions.length, 64, 'real dispatcher action count');
