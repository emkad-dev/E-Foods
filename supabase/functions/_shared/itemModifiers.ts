  ...
  return raw
    ...
    .map((group) => {
      ...
      return {
        id: sanitizeText(record.id),
        label: sanitizeOptionalText(record.label),
        max: Number.isFinite(Number(record.max)) ? Number(record.max) : null,
        min: Number.isFinite(Number(record.min)) ? Number(record.min) : null,
...
export const resolveModifierSelections = ({
  ...
  for (const [groupId, bucket] of selectionBuckets.entries()) {
    ...
    const options = parseModifierOptions(group.options);
    const min = Number.isFinite(Number(group.min))
      ? Number(group.min)
      : group.required === true || mode === 'single'
        ? 1
        : 0;
    const max = Number.isFinite(Number(group.max))
      ? Number(group.max)
      : mode === 'single'
        ? 1
