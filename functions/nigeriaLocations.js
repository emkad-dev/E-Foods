...
const nigeriaLgaOptionsByState = {
...
const defaultNigeriaCoordinate = { latitude: 9.0765, longitude: 7.3986 };

const normalizeNigeriaStateName = (state) => {
  ...
  return Object.keys(nigeriaLgaOptionsByState).find((option) => option.toLowerCase() === normalized) ?? trimmed;
};

const getLgaOptionsForState = (state) => {
...
const getNigeriaAreaCoordinate = (state, lga) => {
  const normalizedState = normalizeNigeriaStateName(state);
  ...
  const index = lgas.findIndex((candidate) => candidate.toLowerCase() === String(lga).trim().toLowerCase());
...
module.exports = {
  defaultNigeriaCoordinate,
  getLgaOptionsForState,
  getNigeriaAreaCoordinate,
  nigeriaLgaOptionsByState,
  nigeriaStateCenters,
  nigeriaStateOptions,
  normalizeNigeriaStateName,
};
