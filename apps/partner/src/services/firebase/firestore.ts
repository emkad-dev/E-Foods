...
export const createUserDocument = async (
  ...
  if (requestedRole && requestedRole !== 'restaurant') {
    throw new Error('Partner profiles must be provisioned with the restaurant role.');
  }
...
export const updateUserDocument = async (
  ...
  if (!currentSnapshot.exists()) {
    throw new Error('Partner profile does not exist. Ask an admin to provision this account before updating it.');
  }
