// Pure, Node-testable state machine for the kitchen-display alarm (Task 15 / F1).
// Kept free of React, audio, timers, and storage on purpose -- the board component
// (apps/partner/src/components/KitchenBoard.tsx) and the useKitchenAlarm hook are the
// thin, impure wiring layer that drives expo-audio/expo-keep-awake and AsyncStorage off
// of this reducer's output; they are not where the alarm logic lives. Matches the split
// documented in apps/customer/src/domain/ratingPrompt.ts and
...
export type KitchenAlarmState = {
  ...
  /** Persisted per-device mute flag. Governs audio only -- never the interstitial. */
  muted: boolean;
};
...
export const kitchenAlarmReducer = (state: KitchenAlarmState, event: KitchenAlarmEvent): KitchenAlarmState => {
  switch (event.type) {
    case 'order_arrived':
      ...
      // arming one order's alarm makes the derived isSoundActive/isInterstitialVisible
 ...
 * the full-screen interstitial must keep showing so a muted tablet doesn't silently
 * drop a new order off screen.
 */
export const isInterstitialVisible = (state: KitchenAlarmState): boolean => state.alarming.size > 0;
...
/** The order ids the interstitial should currently list (still alarming, unacknowledged). */
