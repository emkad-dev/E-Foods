...
export const usePushNotifications = () => {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      const [Device, Notifications] = await Promise.all([import('expo-device'), import('expo-notifications')]);
      ...
      const handleNotificationResponse = async (response: any | null) => {
        if (!response) {
          return;
        }
      ...
      void Notifications.getLastNotificationResponseAsync()
        ...
        .catch((error) => {
          console.warn('Unable to inspect the last customer notification response.', error);
        });
    };
