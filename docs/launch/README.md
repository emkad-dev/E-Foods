# Launch Docs

This folder holds the launch-facing planning docs for FEASTY.

## Read First

1. [MVP scope](mvp-scope.md)
2. [Launch marketing](marketing.md)
3. [Launch legal](legal.md)
4. [Publishing follow-up](publishing-follow-up.md)

## Supporting Docs

- [Analytics and monitoring guide](../ANALYTICS_AND_MONITORING_GUIDE.md)
- [Google sign-in setup](../GOOGLE_SIGNIN_SETUP.md)
- [OSM and MapLibre testing](../OSM_MAPLIBRE_TESTING.md)

## Notes

- These docs are written for launch planning and publication.
- The legal pages linked from `legal.md` live in `docs/legal/`.
